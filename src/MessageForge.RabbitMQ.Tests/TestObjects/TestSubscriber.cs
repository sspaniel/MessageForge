...
public sealed class TestSubscriber : 
    ISubscriber<TestSimpleMessage>,
    ISubscriber<TestComplexMessage>,
    ISubscriber<TestExceptionMessage>,
    ISubscriber<MessageForgeError>
{
    ...
    public static IEnumerable<MessageForgeError> Errors() => _errors;

    public static void ClearMessages()
    {
        _simpleMessages.Clear();
        _complexMessages.Clear();
    ...
    private static readonly ConcurrentBag<MessageForgeError> _errors = [];

    public async Task HandleAsync(TestSimpleMessage message, CancellationToken cancellationToken)
    {
        _simpleMessages.Add(message);
    }

    public async Task HandleAsync(TestComplexMessage message, CancellationToken cancellationToken)
    {
        _complexMessages.Add(message);
    }

    public async Task HandleAsync(TestExceptionMessage message, CancellationToken cancellationToken)
    {
        _exceptionMessages.Add(message);
        throw new NotImplementedException();
    }

    public async Task HandleAsync(MessageForgeError error, CancellationToken cancellationToken)
    {
        _errors.Add(error);
    }
}
