...
public sealed class LifecycleLoggingHookTests
{
    ...
    public void AddMessageForgeRabbitMQ_Registers_Default_Logging_Hooks()
    {
        ...
        // assert
        options.BeforeMessagePublishHooks.Count.ShouldBe(1);
        options.BeforeMessageHandleHooks.Count.ShouldBe(1);
        options.OnMessagePublishErrorHooks.Count.ShouldBe(1);
    }

    [Test]
    public async Task Default_Logging_Hook_Runs_Before_User_Hook()
    {
        // arrange
        var options = new MessageServiceOptions();
